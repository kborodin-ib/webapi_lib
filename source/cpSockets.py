    ...
    try:
        ...
        async with websockets.connect("wss://" + local_ip + "/v1/api/ws", ssl=ssl_context) as websocket:
            ...
            if queue == []:
                ...
                if type(msg) is str:
                    logging.info(f"Adding new {msg} request to an empty queue")
                    queue.extend(msg)

                if type(msg) is list and len(msg) != 0:
                    logging.info(f"Adding multiple new queries {msg} to an empty queue")
                    queue.extend(msg)
